...
// ============================================================================
// LocationTracker.ts — FauxRox GPS Distance Tracking
...
export class LocationTracker extends BaseScriptComponent {
  ...
  private lastGpsUpdateTime: number = 0;  // When GPS last gave us a position update
  ...
  /**
   * Resume tracking without resetting distance (for pause/resume)
   * @param onDistanceUpdate Called with (totalDistance, deltaDistance) in meters
   */
  resumeTracking(onDistanceUpdate?: (totalDist: number, deltaDist: number) => void): void {
    if (this.isTracking) {
      this.log('Already tracking');
      return;
    }
    ...
    this.isTracking = true;
    // Don't reset totalDistance - continue from where we left off
    this.lastPosition = null;  // Reset last position to avoid big jump
    this.onDistanceUpdateCallback = onDistanceUpdate;
    this.lastUpdateTime = getTime();
    ...
    this.log('Resumed tracking. Current distance: ' + this.totalDistance.toFixed(2) + 'm');
  }
   ...
   * Returns: 'CHECKING' | 'CONNECTED' | 'NOT_AVAILABLE' | 'PERMISSION_DENIED'
   */
  getGpsStatus(): string {
   ...
   */
  getGpsStatusMessage(): string {
    return this._gpsStatusMessage;
  }

  /**
   * Check if GPS is connected and working
   */
  isGpsConnected(): boolean {
    return this._gpsStatus === 'CONNECTED';
  }

  /**
   * Check if GPS is actively providing updates (got update within last N seconds)
   * This is more reliable than isLocationReady() for detecting actual signal
   */
  isGpsActivelyUpdating(maxAgeSeconds: number = 3.0): boolean {
    if (!this.isTracking || this.lastGpsUpdateTime === 0) {
      return false;
    }
    var age = getTime() - this.lastGpsUpdateTime;
    return age < maxAgeSeconds;
  }

  /**
   * Set callback for GPS status changes
   * @param callback Called with (status, message) when GPS status changes
   */
  onGpsStatusChange(callback: (status: string, message: string) => void): void {
    this.onGpsStatusChangeCallback = callback;

    // Immediately call with current status
    if (callback) {
      callback(this._gpsStatus, this._gpsStatusMessage);
    }
  }
  ...
  private onUpdate(): void {
    ...
    this.locationService.getCurrentPosition(
      ...
      (_error: string) => {
        this._consecutiveErrors++;
        if (this._consecutiveErrors >= this._maxConsecutiveErrors && this._gpsStatus === 'CONNECTED') {
          this.setGpsStatus('NOT_AVAILABLE', 'GPS signal lost - using step tracking');
        }
      }
    );
  }
  ...
  private handlePositionUpdate(pos: GeoPosition): void {
    ...
    if (dist >= this.minMovementThreshold) {
      ...
      this.lastPosition = pos;
      // ONLY update timestamp when we get REAL movement (not cached/stale position)
      this.lastGpsUpdateTime = getTime();
