...
export class CloudManager extends BaseScriptComponent {
  ...
  async saveRace(record: RaceRecord): Promise<boolean> {
    if (!this.client) {
  ...
  async getAIContext(friendUserId?: string): Promise<string> {
    ...
    if (pb) {
      context += 'Personal Best: ' + (pb.totalTime / 1000).toFixed(1) + 's\n';
      context += 'PB Splits: ' + pb.splits.map(s => s.name + ': ' + (s.duration / 1000).toFixed(1) + 's').join(', ') + '\n';
    }
    ...
    if (friendUserId) {
      ...
      if (friend) {
        context += '\nFriend ' + friend.displayName + ' Best: ' + (friend.totalTime / 1000).toFixed(1) + 's\n';
        context += 'Friend Splits: ' + friend.splits.map(s => s.name + ': ' + (s.duration / 1000).toFixed(1) + 's').join(', ') + '\n';
      }
    }
