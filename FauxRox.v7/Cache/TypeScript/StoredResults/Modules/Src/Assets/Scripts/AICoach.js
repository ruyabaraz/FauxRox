...
let AICoach = (() => {
    ...
    var AICoach = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.BASE_INSTRUCTIONS = `You are a HYROX fitness coach on AR glasses.
...
- SQUAT TARGET REACH: Squat down with control, then stand and reach arms up to touch target. Full depth squat, explosive stand.`;
        ...
        get isBusy() { return this.isAISpeaking || this.isProcessing || this.isRecording; }
        get isSpeaking() { return this.isAISpeaking; }
        ...
        async handleCompareWithUser(targetName) {
        ...
        speakRaceSummary(raceData) {
            ...
            if (raceData.splits.length > 0) {
                ...
                for (const split of raceData.splits) {
                    if (split.duration > slowest.duration)
                        slowest = split;
                    if (split.duration < fastest.duration)
                        fastest = split;
                }
         ...
         */
        speakFormReminder(exerciseName) {
            if (!this.geminiLive || !this.isSessionReady) {
        ...
        buildContext() {
            ...
            if (this.raceStateMachine) {
                ...
                const config = this.raceStateMachine.currentConfig;
