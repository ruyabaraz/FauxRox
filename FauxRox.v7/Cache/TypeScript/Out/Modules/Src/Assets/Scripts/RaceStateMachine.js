...
(function (RaceState) {
    ...
    RaceState["APPROACHING_STATION"] = "APPROACHING_STATION";
    RaceState["APPROACHING_FINISH"] = "APPROACHING_FINISH";
...
let RaceStateMachine = (() => {
    ...
    var RaceStateMachine = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.repPopupPrefab = this.repPopupPrefab;
            // ── Finish Panel UI ─────────────────────────────────────────────────────────
            ...
            this.finishPBBadge = this.finishPBBadge;
            /** PB difference text - e.g. "-0:32" */
            this.finishPBText = this.finishPBText;
            ...
            this.finishFastestText = this.finishFastestText;
            /** Split Insights - Needs work station text */
            this.finishNeedsWorkText = this.finishNeedsWorkText;
            /** Race Again button - primary action */
            this.finishRaceAgainButton = this.finishRaceAgainButton;
            /** View Splits button - secondary action */
            this.finishViewSplitsButton = this.finishViewSplitsButton;
            ...
            this.splitsPanel = this.splitsPanel;
            /** Splits panel status text - "FINISHED!" or "STOPPED" */
            this.splitsStatusText = this.splitsStatusText;
            /** Splits panel time text - total race time */
            this.splitsTimeText = this.splitsTimeText;
            /** Splits panel list text - all split details */
            this.splitsListText = this.splitsListText;
            /** Splits panel Race Again button */
            this.splitsRaceAgainButton = this.splitsRaceAgainButton;
            /** Splits panel Go Back button - returns to finish panel */
            this.splitsGoBackButton = this.splitsGoBackButton;
            // ── Settings ──────────────────────────────────────────────────────────────
            ...
            this._incompleteStations = []; // Stations not completed (for splits panel)
            // Current station progress
            this._currentConfig = null;
            this._stationProgress = 0;
            this._stationRequirement = 0;
            this._stationCompleting = false; // Guard against double completion
            ...
            this._previousGateDot = 1; // For crossing detection (start positive = behind gate)
            this.GATE_HALF_WIDTH = 100; // 100cm = half gate width
            this.PROXIMITY_FALLBACK = 70; // 70cm fallback distance
            // Title fade out
            ...
            this.BURPEE_JUMP_DISTANCE = 10; // 50cm forward = valid jump
            ...
            // Thresholds (in cm)
            this.BURPEE_DROP_THRESHOLD = 20; // Head must drop 20cm
            ...
            this.COUNTDOWN_GO_ZOOM_SCALE = 0.8; // GO! için daha küçük
            ...
            this._repPopupStartPos = null;
            this._repPopupStartScale = null;
            this.REP_POPUP_POP_DURATION = 0.3; // Quick pop in
            this.REP_POPUP_FLOAT_DURATION = 1.2; // Float up and fade
            this.REP_POPUP_TOTAL_DURATION = 1.5; // Total animation time
            this.REP_POPUP_FLOAT_DISTANCE = 50; // cm to float up
            this.REP_POPUP_POP_SCALE = 1.3; // Pop scale multiplier
        ...
        get isZoneHitActive() {
            return this._state === RaceState.STATION &&
                this._currentConfig !== null &&
                this._currentConfig.mode === CourseManager_1.StationMode.ZONE_HIT;
        }
        get splits() {
            const result = [];
        ...
        bindFinishButtons() {
            ...
            if (this.finishRaceAgainButton) {
                var raceAgainBtn = this.finishRaceAgainButton;
            ...
            if (this.finishViewSplitsButton) {
                var viewSplitsBtn = this.finishViewSplitsButton;
            ...
            if (this.splitsRaceAgainButton) {
                var splitsRaceAgainBtn = this.splitsRaceAgainButton;
            ...
            if (this.splitsGoBackButton) {
                var splitsGoBackBtn = this.splitsGoBackButton;
        ...
        onBLEFlowComplete() {
            ...
            // Check if already calibrated (Race Again case)
            var setup = this.setup();
            if (setup && setup.isCalibrated) {
                // Already calibrated - show start button directly
                print('[RaceStateMachine] Already calibrated — ready to start');
        ...
        populateSplitInsights() {
            ...
            for (var i = 0; i < this._splitNames.length; i++) {
                ...
                if (name.toLowerCase().indexOf('run') === 0)
                    continue;
        ...
        checkAndShowPB(currentTotalMs) {
            ...
            var cloud = this.cloudManager;
            if (cloud.getPersonalBest) {
                cloud.getPersonalBest().then((pb) => {
                    if (!pb) {
         ...
         */
        showPBBadge(diffMs, isFirstRace) {
            if (this.finishPBBadge) {
         ...
         */
        storeIncompleteStations(incompleteStations) {
        ...
        togglePause() {
            if (this._state === RaceState.RUNNING || this._state === RaceState.STATION || this._state === RaceState.APPROACHING_STATION || this._state === RaceState.APPROACHING_FINISH) {
                // PAUSE
                ...
                if (this._pausedFromState === RaceState.RUNNING) {
                    ...
                    if (this.statusText) {
                        this.statusText.text = 'Cross the finish line!';
                    }
                }
        ...
        resetRace() {
            ...
            if (this.needsOnboarding()) {
                ...
                // No onboarding - show start button immediately
                if (this.startButtonObject) {
                    this.startButtonObject.enabled = true;
                }
                if (this.instructionText) {
        ...
        spawnRepPopup() {
            ...
            var spawnPos = new vec3(camPos.x + camForward.x * 80, // 80cm in front
            ...
            var lookDir = new vec3(camPos.x - spawnPos.x, 0, camPos.z - spawnPos.z).normalize();
        ...
        updateRepPopup(dt) {
            ...
            // Phase 2: Float up and fade (POP_DURATION to end)
                ...
                var newPos = new vec3(this._repPopupStartPos.x, this._repPopupStartPos.y + floatOffset, this._repPopupStartPos.z);
        ...
        checkStationGateCrossing() {
            ...
            var toPlayer = new vec3(playerPos.x - this._gatePos.x, 0, playerPos.z - this._gatePos.z);
        ...
        prepareForNextStation() {
            ...
            if (this._currentConfig.isFinish) {
                ...
                this._gatePos = new vec3(playerPos.x + playerForward.x * 200, // 2m ahead
                playerPos.y, playerPos.z + playerForward.z * 200);
