...
export class HeartRateTracker extends BaseScriptComponent {
  ...
  /**
   * Update max heart rate (called after onboarding with age-based calculation)
   */
  updateMaxHeartRate(maxHR: number): void {
    if (maxHR > 0 && maxHR <= 220) {
      this.maxHeartRate = maxHR;
      this.log('Max HR updated to: ' + maxHR);
    }
  }
