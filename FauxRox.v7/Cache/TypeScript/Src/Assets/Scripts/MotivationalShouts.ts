...
export class MotivationalShouts extends BaseScriptComponent {
  ...
  private onStateChange(from: string, to: string, stationIndex: number): void {
    ...
    // Race finished - speak detailed summary
    if (to === 'FINISHED' && from !== 'IDLE') {
      this.speakRaceSummary();
    }
  }
  ...
  private speakRaceSummary(): void {
    ...
    this.log('Speaking race summary...');
    (this.aiCoach as any).speakRaceSummary(raceData);
  }
