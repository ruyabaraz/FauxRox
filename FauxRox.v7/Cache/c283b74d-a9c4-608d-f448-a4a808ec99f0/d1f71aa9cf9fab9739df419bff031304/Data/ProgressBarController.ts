...
export class ProgressBarController extends BaseScriptComponent {
  ...
  setProgress(newProgress: number) {
    ...
    // Update bar fill - bar's right edge follows pointer position
    if (this.barScreenTransform) {
      // Set left anchor to match start position (once)
      this.barScreenTransform.anchors.left = this.startPos.x
      // Right edge matches pointer position
      this.barScreenTransform.anchors.right = newPointerPosition
    }
