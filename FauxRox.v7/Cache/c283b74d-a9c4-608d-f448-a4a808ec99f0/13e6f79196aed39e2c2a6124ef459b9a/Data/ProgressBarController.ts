...
export class ProgressBarController extends BaseScriptComponent {
  ...
  setProgress(newProgress: number) {
    ...
    // Update bar fill - scale from original left to right based on progress
    if (this.barScreenTransform) {
      // Keep left at original position
      this.barScreenTransform.anchors.left = this.barLeftAnchor
      // Right edge interpolates from left (0%) to original right (100%)
      this.barScreenTransform.anchors.right = MathUtils.lerp(this.barLeftAnchor, this.barRightAnchor, newProgress)
    }
