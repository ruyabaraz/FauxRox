...
export class BLEConnectionUI extends BaseScriptComponent {
  ...
  private populateDeviceList(): void {
    ...
    var buttonSpacing = 4.0;  // cm between buttons (adjust as needed)
    ...
    for (var i = 0; i < devices.length; i++) {
      ...
      // Position button vertically (stack downward)
      var transform = btn.getTransform();
      var localPos = transform.getLocalPosition();
      transform.setLocalPosition(new vec3(localPos.x, localPos.y - (i * buttonSpacing), localPos.z));
