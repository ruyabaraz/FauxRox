...
export class CoachToggleButton extends BaseScriptComponent {
  ...
  private onIconScale: vec3 = null;
  private offIconScale: vec3 = null;
  ...
  private updateIcons(isOn: boolean): void {
    ...
    if (this.onIcon && this.onIconScale) {
      const scale = isOn ? this.onIconScale : new vec3(0, 0, 0);
      this.onIcon.getTransform().setLocalScale(scale);
      print('[CoachToggleButton] onIcon visible = ' + isOn);
    }
    if (this.offIcon && this.offIconScale) {
      const scale = isOn ? new vec3(0, 0, 0) : this.offIconScale;
      this.offIcon.getTransform().setLocalScale(scale);
      print('[CoachToggleButton] offIcon visible = ' + !isOn);
    }
  }
}
