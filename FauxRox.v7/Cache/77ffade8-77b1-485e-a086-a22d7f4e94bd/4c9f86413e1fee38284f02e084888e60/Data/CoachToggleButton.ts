...
export class CoachToggleButton extends BaseScriptComponent {
  ...
  private setIconState(isOn: boolean): void {
    ...
    if (this.microphoneOn) {
      // Disable both the object AND all visual components
      this.microphoneOn.enabled = isOn;
      const image = this.microphoneOn.getComponent('Component.Image');
      if (image) {
        image.enabled = isOn;
        print('[CoachToggleButton] MicrophoneON Image.enabled = ' + isOn);
      }
      const rmv = this.microphoneOn.getComponent('Component.RenderMeshVisual');
      if (rmv) {
        rmv.enabled = isOn;
        print('[CoachToggleButton] MicrophoneON RMV.enabled = ' + isOn);
      }
    }
    ...
    if (this.microphoneOff) {
      ...
      const image = this.microphoneOff.getComponent('Component.Image');
      if (image) {
        image.enabled = !isOn;
        print('[CoachToggleButton] MicrophoneOFF Image.enabled = ' + !isOn);
      }
      const rmv = this.microphoneOff.getComponent('Component.RenderMeshVisual');
      if (rmv) {
        rmv.enabled = !isOn;
        print('[CoachToggleButton] MicrophoneOFF RMV.enabled = ' + !isOn);
      }
    }
  }
}
