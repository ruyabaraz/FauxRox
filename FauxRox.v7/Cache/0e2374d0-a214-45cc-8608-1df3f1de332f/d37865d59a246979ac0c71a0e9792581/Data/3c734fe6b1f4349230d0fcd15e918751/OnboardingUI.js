...
let OnboardingUI = (() => {
    ...
    var OnboardingUI = _classThis = class extends _classSuper {
        ...
        onEditName() {
            if (this._isRecordingName)
                return;
            this._isRecordingName = true;
            this.log('Listening for name...');
            if (this.nameText) {
                this.nameText.text = 'Say your name...';
            }
            var settings = AsrModule.AsrTranscriptionOptions.create();
            settings.mode = AsrModule.AsrMode.HighAccuracy;
            settings.silenceUntilTerminationMs = 2000;
            settings.onTranscriptionUpdateEvent.add((output) => {
                if (output.isFinal && output.text && output.text.trim().length > 0) {
                    this._displayName = output.text.trim();
                    this.log('Name set: ' + this._displayName);
                    this._isRecordingName = false;
        ...
        onConfirm() {
            ...
            if (this.profileManager) {
                var profile = this.profileManager.createProfile(this._displayName, this._birthYear, this._fitnessLevel, this._goal, false, null);
                this.hide();
