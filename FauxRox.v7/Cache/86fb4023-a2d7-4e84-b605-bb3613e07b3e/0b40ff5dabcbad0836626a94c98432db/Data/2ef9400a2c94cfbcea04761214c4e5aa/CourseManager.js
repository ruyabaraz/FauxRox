...
let CourseManager = (() => {
    ...
    var CourseManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.overheadPullPrefab = this.overheadPullPrefab; // Station 1: Overhead Pull Gates
            this.powerLanePrefab = this.powerLanePrefab; // Station 2: Power Lane
            this.crabWalkPrefab = this.crabWalkPrefab; // Station 3: Crab Walk
            this.burpeeBroadJumpPrefab = this.burpeeBroadJumpPrefab; // Station 4: Burpee Broad Jump
            this.powerRowPrefab = this.powerRowPrefab; // Station 5: Power Row Gates
            this.heavyCarryPrefab = this.heavyCarryPrefab; // Station 6: Heavy Carry Lane
            this.walkingLungesPrefab = this.walkingLungesPrefab; // Station 7: Walking Lunges
            this.targetPressPrefab = this.targetPressPrefab; // Station 8: Target Press
            ...
            this.directionArrowPrefab = this.directionArrowPrefab;
            ...
            /** Progressive reveal: only show current + next station */
            this.progressiveReveal = this.progressiveReveal;
            // ── Settings ─────────────────────────────────────────────────────────────
            /** Run distance scale: 1.0 = full 1km, 0.25 = 250m per segment */
            ...
            this._revealedStations = [];
            this._directionArrow = null;
            this._forwardDir = vec3.forward();
            this._lateralDir = vec3.right();
            this._courseOrigin = vec3.zero();
        ...
        setStationActive(index) {
            if (index >= 0 && index < this.stationObjects.length && this.stationObjects[index] && this.activeMaterial) {
                this.applyMaterial(this.stationObjects[index], this.activeMaterial);
            }
        }
        /**
         * Reveal the next station (for progressive reveal mode)
         */
        revealStation(index) {
            if (index < 0 || index >= this.stationConfigs.length)
                return;
            if (this._revealedStations[index])
                return; // Already revealed
            var pos = this.stationPositions[index];
            var obj = this.spawnStationVisual(index, pos, this._forwardDir);
            this.stationObjects[index] = obj;
            this._revealedStations[index] = true;
            // Apply highlight material to new station
            if (obj && this.highlightMaterial) {
                this.applyMaterial(obj, this.highlightMaterial);
            }
            print('[CourseManager] Revealed station ' + index + ': ' + this.stationNames[index]);
        }
        /**
         * Update direction arrow to point toward target station
         * Arrow floats in front of player and rotates to point at next station
         */
        updateDirectionArrow(playerPos, targetStationIndex, cameraTransform) {
            if (!this._directionArrow)
                return;
            if (targetStationIndex < 0 || targetStationIndex >= this.stationPositions.length) {
                this._directionArrow.enabled = false;
                return;
            }
            var targetPos = this.stationPositions[targetStationIndex];
            // Calculate direction to target (horizontal only)
            var dirToTarget = new vec3(targetPos.x - playerPos.x, 0, targetPos.z - playerPos.z);
            var distToTarget = dirToTarget.length;
            if (distToTarget < 1) {
                // Too close to target, hide arrow
                this._directionArrow.enabled = false;
                return;
            }
            dirToTarget = dirToTarget.normalize();
            // Position arrow in front of player, offset toward target direction
            // Arrow floats 150cm in front, 80cm above ground
            var arrowPos = new vec3(playerPos.x + dirToTarget.x * 150, playerPos.y + 80, playerPos.z + dirToTarget.z * 150);
            this._directionArrow.getTransform().setWorldPosition(arrowPos);
            // Rotate arrow to point at target
            // quat.lookAt(direction, up) makes object's -Z face the direction
            // We apply a 180° Y rotation so +Z (arrow tip) points at target
            var baseRotation = quat.lookAt(dirToTarget, vec3.up());
            var flipY = quat.angleAxis(Math.PI, vec3.up()); // 180° around Y
            var rotation = baseRotation.multiply(flipY);
            this._directionArrow.getTransform().setWorldRotation(rotation);
            // Scale arrow based on distance (bigger = farther)
            var rawScale = distToTarget / 500;
            var scale = Math.max(0.5, Math.min(2.0, rawScale));
            this._directionArrow.getTransform().setLocalScale(new vec3(scale, scale, scale));
            this._directionArrow.enabled = true;
        }
        /**
        ...
        /**
         * Get distance from player to target station
         */
        getDistanceToStation(playerPos, stationIndex) {
            if (stationIndex < 0 || stationIndex >= this.stationPositions.length) {
                return 0;
            }
            var targetPos = this.stationPositions[stationIndex];
            var dx = targetPos.x - playerPos.x;
            var dz = targetPos.z - playerPos.z;
            return Math.sqrt(dx * dx + dz * dz);
        }
        resetCourse() {
            // Destroy station objects
            for (var i = 0; i < this.stationObjects.length; i++) {
                if (this.stationObjects[i]) {
                    this.stationObjects[i].destroy();
                }
            }
            // Destroy direction arrow
            if (this._directionArrow) {
                this._directionArrow.destroy();
                this._directionArrow = null;
            }
            ...
            this.stationObjects = [];
            this.stationCompleted = [];
            this._revealedStations = []; // Was missing!
            this.isCoursePlaced = false;
            this.stationCount = 0;
            this.currentRunTarget = 0;
            this.currentRunDistance = 0;
            // Reset course layout state
            this._forwardDir = vec3.forward();
            this._lateralDir = vec3.right();
            this._courseOrigin = vec3.zero();
            print('[CourseManager] Course reset (full cleanup)');
        }
        // ── Layout ───────────────────────────────────────────────────────────────
        layoutStations(origin, forwardDir, lateralDir) {
            // Store for later station reveals
            this._courseOrigin = new vec3(origin.x, origin.y, origin.z);
            this._forwardDir = forwardDir;
            this._lateralDir = lateralDir;
            ...
            for (var i = 0; i < this.stationConfigs.length; i++) {
                ...
                // Calculate position but don't spawn yet if progressive reveal
                ...
                this._revealedStations.push(false);
                if (this.progressiveReveal) {
                    // Only spawn START (index 0) initially
                    if (i === 0) {
                        var obj = this.spawnStationVisual(i, currentPos, forwardDir);
                        this.stationObjects.push(obj);
                        this._revealedStations[i] = true;
                    }
                    else {
                        this.stationObjects.push(null); // Will be spawned later
                    }
                }
                else {
                    // Spawn all stations immediately (old behavior)
                    var obj = this.spawnStationVisual(i, currentPos, forwardDir);
                    this.stationObjects.push(obj);
                    this._revealedStations[i] = true;
                }
            ...
            if (this.highlightMaterial && this.stationObjects.length > 0 && this.stationObjects[0]) {
                this.applyMaterial(this.stationObjects[0], this.highlightMaterial);
            }
            // Spawn direction arrow
            this.spawnDirectionArrow();
            print('[CourseManager] Course placed — ' + this.stationCount + ' stations');
            if (this.progressiveReveal) {
                print('[CourseManager] Progressive reveal enabled — only START visible');
            }
            this.printCourseSummary();
        }
        spawnStationVisual(index, pos, forwardDir) {
            var cfg = this.stationConfigs[index];
        ...
        spawnDirectionArrow() {
            if (!this.directionArrowPrefab)
                return;
            this._directionArrow = this.directionArrowPrefab.instantiate(null);
            this._directionArrow.enabled = false; // Hidden until needed
            print('[CourseManager] Direction arrow spawned');
        }
        printCourseSummary() {
            print('═══════════════════════════════════════════');
