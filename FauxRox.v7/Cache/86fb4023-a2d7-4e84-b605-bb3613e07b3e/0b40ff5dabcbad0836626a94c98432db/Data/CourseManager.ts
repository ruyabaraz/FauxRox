...
export class CourseManager extends BaseScriptComponent {
  ...
  @input @allowUndefined directionArrowPrefab: ObjectPrefab;
  ...
  @input progressiveReveal: boolean = true;

  // ── Settings ─────────────────────────────────────────────────────────────

  /** Run distance scale: 1.0 = full 1km, 0.25 = 250m per segment */
  @input runDistanceScale: number = 0.25;
  ...
  private _revealedStations: boolean[] = [];
  private _directionArrow: SceneObject = null;
  private _forwardDir: vec3 = vec3.forward();
  private _lateralDir: vec3 = vec3.right();
  private _courseOrigin: vec3 = vec3.zero();
  ...
  highlightStation(index: number): void {
    for (var i = 0; i < this.stationObjects.length; i++) {
      if (!this.stationObjects[i]) continue;  // Skip null objects
   ...
   */
  revealStation(index: number): void {
    if (index < 0 || index >= this.stationConfigs.length) return;
    if (this._revealedStations[index]) return;  // Already revealed
   ...
   */
  updateDirectionArrow(playerPos: vec3, targetStationIndex: number, cameraTransform?: Transform): void {
    if (!this._directionArrow) return;
    ...
    var arrowPos = new vec3(
      playerPos.x + dirToTarget.x * 150,
      playerPos.y + 80,
      playerPos.z + dirToTarget.z * 150
    );
   ...
   */
  getDistanceToStation(playerPos: vec3, stationIndex: number): number {
  ...
  private spawnStationVisual(index: number, pos: vec3, forwardDir: vec3): SceneObject {
  ...
  private spawnDirectionArrow(): void {
    if (!this.directionArrowPrefab) return;
