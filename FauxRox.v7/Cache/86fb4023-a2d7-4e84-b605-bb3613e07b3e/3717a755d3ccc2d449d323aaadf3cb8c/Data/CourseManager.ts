...
export class CourseManager extends BaseScriptComponent {
  ...
  private disableParticleEmitters(obj: SceneObject): void {
    // Try to get any visual component and disable it
    var components = obj.getComponents() as any[];
    for (var i = 0; i < components.length; i++) {
      var comp = components[i];
      // Check if it's a particle-related component
      if (comp && comp.getTypeName &&
          (comp.getTypeName().indexOf('Particle') >= 0 || comp.getTypeName().indexOf('VFX') >= 0)) {
        comp.enabled = false;
        print('[CourseManager] Disabled: ' + comp.getTypeName());
      }
    }
