...
export class CourseManager extends BaseScriptComponent {
  ...
  private fadeIn(obj: SceneObject, maxAlpha: number = 1.0): void {
    // Start invisible
    this.setObjectAlpha(obj, 0);
    obj.enabled = true;

    // Animate to visible (up to maxAlpha)
    ...
    fadeEvent.bind(() => {
      ...
      this.setObjectAlpha(obj, t * maxAlpha);
  ...
  private setObjectAlpha(obj: SceneObject, alpha: number): void {
    // Try to set alpha on all RenderMeshVisuals in hierarchy
    this.forEachRenderMesh(obj, (rmv: RenderMeshVisual) => {
    ...
    var textComponents = obj.getComponents('Component.Text') as Text[];
    for (var i = 0; i < textComponents.length; i++) {
      try {
        var tc = textComponents[i] as any;
  ...
  private forEachRenderMesh(obj: SceneObject, callback: (rmv: RenderMeshVisual) => void): void {
    // Check this object
