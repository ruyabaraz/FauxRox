...
let CourseManager = (() => {
    ...
    var CourseManager = _classThis = class extends _classSuper {
        ...
        /**
         * Update direction arrow - placed AT the target station, facing the player
         * This is a "beacon" style indicator, not a floating HUD element
         */
        updateDirectionArrow(playerPos, targetStationIndex) {
            ...
            // Hide if too close (within 100cm / 1m)
            if (distToTarget < 100) {
                this._directionArrow.enabled = false;
                return;
            }
            // Position arrow AT the target station, slightly above ground
            var arrowPos = new vec3(targetPos.x, targetPos.y + 100, // 100cm above station ground level
            targetPos.z);
            this._directionArrow.getTransform().setWorldPosition(arrowPos);
            // Rotate arrow to face the player (billboard style)
            var dirToPlayer = new vec3(playerPos.x - targetPos.x, 0, // Keep horizontal
            playerPos.z - targetPos.z).normalize();
            if (dirToPlayer.length > 0.01) {
                var rotation = quat.lookAt(dirToPlayer, vec3.up());
                this._directionArrow.getTransform().setWorldRotation(rotation);
            }
            // Scale based on distance - larger when farther so it's visible
            var rawScale = Math.max(1.0, distToTarget / 300);
            var scale = Math.min(3.0, rawScale);
