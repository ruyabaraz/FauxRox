...
import { AlphaOverride } from './AlphaOverride';
...
export interface StationConfig {
  ...
  maxAlpha?: number;          // Max alpha for fade-in (0.0-1.0, default 1.0)
}
...
export class CourseManager extends BaseScriptComponent {
  ...
  private _activeStationMaxAlpha: number = 1.0;
  ...
  private fadeIn(obj: SceneObject, defaultMaxAlpha: number = 1.0): void {
    // Start invisible
  ...
  private fadeOut(obj: SceneObject, defaultMaxAlpha: number = 1.0, onComplete?: () => void): void {
    var startTime = getTime();
    var duration = this.fadeDuration;
   ...
   */
  private getMaxAlphaOverride(obj: SceneObject): number | null {
    var override = obj.getComponent(AlphaOverride.getTypeName()) as AlphaOverride;
    if (override) {
      return override.maxAlpha;
    }
    return null;
  }

  private setObjectAlpha(obj: SceneObject, fadeProgress: number, defaultMaxAlpha: number = 1.0): void {
    // Set alpha on all RenderMeshVisuals in hierarchy
    // Each mesh can have its own maxAlpha via name suffix (e.g., "_a30" = 0.30)
    this.forEachRenderMesh(obj, (rmv: RenderMeshVisual, meshObj: SceneObject) => {
      // Per-mesh alpha override from name, fallback to station default
      var meshMaxAlpha = this.getMaxAlphaFromName(meshObj);
    ...
    // Handle Text components (use default alpha)
    this.forEachText(obj, (textComp: any, textObj: SceneObject) => {
      var textMaxAlpha = this.getMaxAlphaFromName(textObj);
