...
let CourseManager = (() => {
    ...
    var CourseManager = _classThis = class extends _classSuper {
        ...
        updateDirectionArrow(playerPos, targetStationIndex) {
            ...
            var arrowPos = new vec3(playerPos.x, playerPos.y + 50, // 50cm above ground
            playerPos.z);
            this._directionArrow.getTransform().setWorldPosition(arrowPos);
            // Rotate arrow to point at target
            var direction = new vec3(targetPos.x - playerPos.x, 0, // Keep horizontal
            targetPos.z - playerPos.z).normalize();
            if (direction.length > 0.01) {
                this._directionArrow.getTransform().setWorldRotation(quat.lookAt(direction, vec3.up()));
            }
