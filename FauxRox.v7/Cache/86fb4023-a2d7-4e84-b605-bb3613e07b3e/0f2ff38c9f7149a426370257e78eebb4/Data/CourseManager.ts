...
export class CourseManager extends BaseScriptComponent {
  ...
  /**
   * Extract maxAlpha from mesh name suffix: "MeshName_a30" → 0.30
   * Returns null if no suffix found (use default)
   */
  private getMaxAlphaFromName(obj: SceneObject): number | null {
    var name = obj.name;
    var match = name.match(/_a(\d+)$/);
    if (match) {
