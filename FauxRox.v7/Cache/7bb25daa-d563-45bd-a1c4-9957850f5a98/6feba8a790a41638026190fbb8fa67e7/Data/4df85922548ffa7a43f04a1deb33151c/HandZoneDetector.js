...
let HandZoneDetector = (() => {
    ...
    var HandZoneDetector = _classThis = class extends _classSuper {
        ...
        processOverheadReach(hands, camPos) {
            ...
            // Check if EITHER hand hits target zone (not both - too strict for overhead reach)
            var leftDist = hands.leftPos.distance(this._targetPosition);
            var rightDist = hands.rightPos.distance(this._targetPosition);
            ...
            var hitTarget = minDist <= effectiveRadius;
            // Debug log every ~30 frames
            if (Math.random() < 0.03) {
                this.log('OVERHEAD_REACH: avgY=' + avgY.toFixed(0) + 'cm, minDist=' + minDist.toFixed(0) + 'cm, radius=' + effectiveRadius.toFixed(0) + 'cm, state=' + this._state);
            }
            switch (this._state) {
