...
// ============================================================================
// HandZoneDetector.ts — FauxRox Hand Zone Detection
// Lens Studio 5.x · Spectacles · TypeScript
// ============================================================================
// Tracks hand positions and detects when hands enter target zones
// Used for Target Press, Air SkiErg, Power Row Gates
// ============================================================================
...
let HandZoneDetector = (() => {
    ...
    var HandZoneDetector = _classThis = class extends _classSuper {
        constructor() {
            ...
            this._targetObject = null; // External target object (e.g., sphere for squat press)
            this._targetObjectRadius = 0; // Radius of target object (from scale)
            // Callbacks
         ...
         * @param targetObject - Optional external target object (sphere for squat press)
         */
        startDetection(motionType, onRep, onStateChange, stationWorldPos, targetObject) {
            ...
            // Calculate target object radius from scale (assuming uniform scale sphere)
            if (this._targetObject && !isNull(this._targetObject)) {
                var scale = this._targetObject.getTransform().getWorldScale();
                this._targetObjectRadius = Math.max(scale.x, scale.y, scale.z) / 2;
                this.log('Target object radius: ' + this._targetObjectRadius.toFixed(1) + 'cm');
            }
            else {
                this._targetObjectRadius = 0;
            }
            // Spawn target zone visual (positioned once, not every frame)
            this.spawnTargetZone();
            var mode = this._targetObject ? 'target-object' : (this._stationAnchor ? 'station-anchored' : 'camera-follow');
            this.log('Detection started: ' + motionType + ' (' + mode + ')');
        }
        /**
         * Resume detection without resetting rep count (for pause/resume)
         */
        resumeDetection(onRep, onStateChange) {
            if (this._isActive) {
                this.log('Already detecting');
                return;
            }
            if (onRep) {
                this._onRepCallback = onRep;
            ...
            // Don't reset _repCount - continue from where we left off
            this._state = ZoneState.WAITING;
            this._isActive = true;
            this._cooldownTimer = 0;
            // Re-spawn target zone
            this.spawnTargetZone();
            this.log('Detection resumed. Current reps: ' + this._repCount);
        }
        /**
         * Stop zone detection
         */
        ...
        updateTargetPositionStationAnchored() {
            ...
            switch (this._motionType) {
                ...
                case CourseManager_1.MotionType.AIR_SKIERG:
        ...
        onUpdate() {
            ...
            if (this._targetObject && !isNull(this._targetObject)) {
                // Use external target object position (e.g., sphere)
                this._targetPosition = this._targetObject.getTransform().getWorldPosition();
            }
            else if (!this._stationAnchor) {
                // Camera-relative targeting
                this.updateTargetPosition();
            }
            // else: station-anchored targets stay fixed (already set in updateTargetPositionStationAnchored)
            // Handle cooldown
            ...
            switch (this._motionType) {
                case CourseManager_1.MotionType.OVERHEAD_REACH:
                    ...
                    break;
                case CourseManager_1.MotionType.AIR_SKIERG:
                    this.processAirSkiErg(hands, camPos);
                    break;
                case CourseManager_1.MotionType.FORWARD_PUSH:
                case CourseManager_1.MotionType.BACKWARD_PULL:
        ...
        processOverheadReach(hands, camPos) {
            ...
            // Check if BOTH hands hit target zone (with 1.5x radius for forgiveness)
            var leftDist = hands.leftPos.distance(this._targetPosition);
            var rightDist = hands.rightPos.distance(this._targetPosition);
            var maxDist = Math.max(leftDist, rightDist); // Both hands must be within radius
            // Use 1.5x radius for overhead reach (more forgiving)
            var effectiveRadius = this._targetObjectRadius > 0
                ? this._targetObjectRadius * 1.5
                : this.targetZoneRadius * 1.5;
            var hitTarget = maxDist <= effectiveRadius;
            // Debug log every ~30 frames
            if (Math.random() < 0.03) {
                this.log('OVERHEAD_REACH: avgY=' + avgY.toFixed(0) + 'cm, L=' + leftDist.toFixed(0) + ' R=' + rightDist.toFixed(0) + ', radius=' + effectiveRadius.toFixed(0) + 'cm, state=' + this._state);
            }
            switch (this._state) {
                case ZoneState.WAITING:
                    if (inReadyZone) {
                        this.setState(ZoneState.READY);
                        this.log('Hands LOW - ready! avgY=' + avgY.toFixed(0));
                    }
                    break;
                ...
                case ZoneState.MOVING:
                    if (hitTarget) {
                        this.log('TARGET HIT! L=' + leftDist.toFixed(0) + ' R=' + rightDist.toFixed(0) + ' <= radius=' + effectiveRadius.toFixed(0) + 'cm');
                        this.onTargetHit();
                    }
                    else if (inReadyZone) {
        ...
        processHorizontalMotion(hands, camPos) {
            ...
            // Forward distance: positive = hands in front of camera, negative = hands behind
            var forwardDist = (handAvgX - camPos.x) * camForward.x + (handAvgZ - camPos.z) * camForward.z;
            // Average hand Y relative to camera
            var avgY = ((hands.leftY + hands.rightY) / 2) - camPos.y;
            if (this._motionType === CourseManager_1.MotionType.BACKWARD_PULL) {
                // ROWING: Ready = arms extended forward, Hit = hands pulled back behind camera
                // Based on actual data: extended ≈ 0 to +7cm, pulled back ≈ -30 to -40cm
                var extendedThreshold = -5; // Arms forward (at or ahead of camera plane)
                var pulledBackThreshold = -20; // Arms pulled back (behind camera plane)
                var validHeight = avgY > -55 && avgY < 10; // Between chest and shoulder height
                var inReadyZone = forwardDist >= extendedThreshold && validHeight;
                var hitTarget = forwardDist <= pulledBackThreshold && validHeight;
                // Debug log every ~30 frames
                if (Math.random() < 0.03) {
                    this.log('BACKWARD_PULL: fwd=' + forwardDist.toFixed(0) + 'cm, avgY=' + avgY.toFixed(0) + 'cm, ready>=' + extendedThreshold + ', hit<=' + pulledBackThreshold + ', state=' + this._state);
                }
                switch (this._state) {
                    case ZoneState.WAITING:
                        if (inReadyZone) {
                            this.setState(ZoneState.READY);
                            this.log('Arms EXTENDED - ready! fwd=' + forwardDist.toFixed(0));
                        }
                        break;
                    case ZoneState.READY:
                        if (!inReadyZone && forwardDist < extendedThreshold) {
                            this.setState(ZoneState.MOVING);
                        }
                        break;
                    case ZoneState.MOVING:
                        if (hitTarget) {
                            this.log('PULL complete! fwd=' + forwardDist.toFixed(0));
                            this.onTargetHit();
                        }
                        else if (inReadyZone) {
                            // Extended again without completing pull - reset
                            this.setState(ZoneState.READY);
                        }
                        break;
                }
                ...
                // FORWARD_PUSH: Ready = hands near body, Hit = hands pushed forward
                ...
                // Debug log every ~30 frames
                if (Math.random() < 0.03) {
                    this.log('FORWARD_PUSH: fwd=' + forwardDist.toFixed(0) + 'cm, inTarget=' + hitTarget + ', state=' + this._state);
                }
                switch (this._state) {
                    case ZoneState.WAITING:
                    case ZoneState.READY:
        ...
        isInTargetZone(handPos) {
            ...
            // Use target object radius if available, otherwise use configured targetZoneRadius
            var radius = this._targetObjectRadius > 0 ? this._targetObjectRadius : this.targetZoneRadius;
            return dist <= radius;
        }
        getHandPositions() {
            var result = {
