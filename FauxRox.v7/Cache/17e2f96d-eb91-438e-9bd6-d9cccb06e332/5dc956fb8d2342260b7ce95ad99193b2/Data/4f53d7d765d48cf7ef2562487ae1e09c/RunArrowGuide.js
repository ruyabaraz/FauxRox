...
let RunArrowGuide = (() => {
    ...
    var RunArrowGuide = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.arrowHeight = this.arrowHeight; // Slightly above ground
            /** Floor Y position (set by calibration) */
            this.floorY = this.floorY;
        ...
        spawnArrowsAhead() {
            ...
            while (this._arrows.length < this.maxArrows &&
                ...
                var arrowPos = new vec3(playerGroundPos.x + direction.x * currentDist, this.floorY + this.arrowHeight, playerGroundPos.z + direction.z * currentDist);
        ...
        spawnArrow(position, direction) {
            ...
            // Arrow should be flat on ground, pointing toward target
            // Using lookAt with up = world up, then rotating to be flat
            var rotation = quat.lookAt(direction, vec3.up());
            // If arrow prefab is already flat (rotated -90 on X), this should work
            // If not, we need to compose with a -90 X rotation
            arrow.getTransform().setWorldRotation(rotation);
