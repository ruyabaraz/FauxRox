...
export class HeartRateTracker extends BaseScriptComponent {
  ...
  private onScanCompleteCallback: () => void = null;
  ...
  private onScanComplete(): void {
    ...
    // If callback is set (BLEConnectionUI), let it handle device selection
    if (this.onScanCompleteCallback) {
      this.onScanCompleteCallback();
      return;
    }

    // Fallback: auto-connect to first HR device (legacy behavior)
  ...
  private registerNotifications(): void {
    this.hrCharacteristic
      ...
      .then(() => {
        ...
        // Start polling as fallback in case notifications don't fire reliably
        this.startPolling();
      })
      .catch((error: string) => {
  ...
  /** Fallback polling - reads HR characteristic every second in case notifications fail */
  private startPolling(): void {
    this.log('Starting HR polling fallback');
    var pollEvent = this.createEvent('UpdateEvent');
    var lastPollTime = 0;
    var pollInterval = 1.0; // Poll every 1 second

    pollEvent.bind(() => {
      if (this._connectionState !== HRConnectionState.CONNECTED) return;
      ...
      this.log('Polling HR characteristic...');

      // Read characteristic value directly
      try {
        this.hrCharacteristic.readValue()
          .then((val: Uint8Array) => {
            this.log('Poll read success, length: ' + (val ? val.length : 'null'));
            if (val && val.length >= 2) {
              this.onHRNotification(val);
            }
          })
          .catch((e: string) => {
            this.log('Poll read error: ' + e);
          });
      } catch (e) {
        this.log('Poll exception: ' + e);
      }
    });
  }

  private onHRNotification(val: Uint8Array): void {
    this.log('HR Notification received, length: ' + (val ? val.length : 'null'));
  ...
  setScanCompleteCallback(callback: () => void): void {
    this.onScanCompleteCallback = callback;
  }

  getFoundDevices(): { name: string, address: Uint8Array }[] {
    return this.foundDevices.map(d => ({
      name: d.deviceName || 'Unknown',
      address: d.deviceAddress
    }));
  }

  connectToDeviceByAddress(address: Uint8Array): void {
    for (var i = 0; i < this.foundDevices.length; i++) {
      var device = this.foundDevices[i];
      if (this.compareAddresses(device.deviceAddress, address)) {
        this.connectToDevice(device);
        return;
      }
    }
    this.setConnectionState(HRConnectionState.ERROR, 'Device not found');
  }

  private compareAddresses(a: Uint8Array, b: Uint8Array): boolean {
