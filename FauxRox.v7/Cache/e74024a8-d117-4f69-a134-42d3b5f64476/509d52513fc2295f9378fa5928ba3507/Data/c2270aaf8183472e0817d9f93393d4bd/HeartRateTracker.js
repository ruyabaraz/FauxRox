...
let HeartRateTracker = (() => {
    ...
    var HeartRateTracker = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.targetDeviceName = this.targetDeviceName;
        ...
        startScan() {
            ...
            // Check if Bluetooth classes are available
            if (typeof Bluetooth === 'undefined' || typeof Bluetooth.ScanFilter === 'undefined') {
                this.log('ERROR: Bluetooth.ScanFilter not available - ensure Bluetooth Central Module asset is added to the project');
                this.setConnectionState(HRConnectionState.ERROR, 'BLE not configured');
                return;
            }
            // Scan without filter to find ALL BLE devices (Echo app may not advertise standard HR UUID)
            var filter;
            var settings;
            try {
                filter = new Bluetooth.ScanFilter();
                // NO serviceUUID filter - scan for everything
                settings = new Bluetooth.ScanSettings();
                settings.uniqueDevices = true;
                settings.timeoutSeconds = this.scanTimeout;
            }
            catch (e) {
                this.log('ERROR creating BLE scan objects: ' + e);
                this.setConnectionState(HRConnectionState.ERROR, 'BLE init failed');
                return;
            }
            this.log('Scanning for ALL BLE devices (no filter)...');
        ...
        onScanResult(result) {
            ...
            // Check if this is a HR device we want to connect to
            var deviceName = result.deviceName || '';
            var upperName = deviceName.toUpperCase();
            // Look for Echo app or any HR-related device
