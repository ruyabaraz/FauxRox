...
export class HeartRateTracker extends BaseScriptComponent {
  ...
  startScan(): void {
    ...
    var scanFilter: Bluetooth.ScanFilter;
    var scanSettings: Bluetooth.ScanSettings;

    try {
      this.log('Creating ScanFilter...');
      scanFilter = new Bluetooth.ScanFilter();
      this.log('ScanFilter created OK');
    } catch (e) {
      this.log('ScanFilter ERROR: ' + e);
      this.setConnectionState(HRConnectionState.ERROR, 'ScanFilter failed');
      return;
    }

    try {
      this.log('Creating ScanSettings...');
      scanSettings = new Bluetooth.ScanSettings();
      this.log('ScanSettings created OK');
    } catch (e) {
      this.log('ScanSettings ERROR: ' + e);
      this.setConnectionState(HRConnectionState.ERROR, 'ScanSettings failed');
      return;
    }
