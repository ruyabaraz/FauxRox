...
Be encouraging but direct. Focus on actionable advice.`;
  ...
  private activateToggleMode(): void {
    ...
    // Show indicator and wave animation immediately
    if (this.recordingIndicator) {
    ...
    // Pulse only starts when user speaks
  ...
  private setListeningAnimation(active: boolean): void {
    // Animation stays visible while toggle is ON
    // This function now only controls pulse (mic pulse handles that)
    // Keeping for compatibility but animation visibility is handled elsewhere
  }
