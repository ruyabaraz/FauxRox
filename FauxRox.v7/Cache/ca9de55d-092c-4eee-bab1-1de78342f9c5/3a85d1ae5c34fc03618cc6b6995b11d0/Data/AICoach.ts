...
Be encouraging but direct. Focus on actionable advice.`;
  ...
  private waveAnimatedTexture: AnimatedTextureFileProvider = null;
  ...
  onAwake(): void {
    ...
    if (this.listeningWaveAnimation) {
      const image = this.listeningWaveAnimation.getComponent('Component.Image') as Image;
      if (image && image.mainPass && image.mainPass.baseTex) {
        this.waveAnimatedTexture = image.mainPass.baseTex as unknown as AnimatedTextureFileProvider;
  ...
  private activateToggleMode(): void {
    ...
    // Show wave animation (paused - will animate when user speaks)
    if (this.listeningWaveAnimation) {
      this.listeningWaveAnimation.enabled = true;
    }
    if (this.waveAnimatedTexture && this.waveAnimatedTexture.pause) {
      this.waveAnimatedTexture.pause();
    }
  ...
  private setListeningAnimation(active: boolean): void {
    if (this.listeningWaveAnimation) {
      this.listeningWaveAnimation.enabled = true; // Always visible when toggle is on
    }

    // Control animation playback
    if (this.waveAnimatedTexture) {
      if (active) {
        // Play looping (-1 = infinite loops)
        (this.waveAnimatedTexture as any).play(-1, 0); // 0 = Loop mode
      } else {
        (this.waveAnimatedTexture as any).pause();
      }
    }
  }
