...
Be encouraging but direct. Focus on actionable advice.`;
   ...
   * Sends directly to Gemini for voice output, no user input needed
   */
  speakShout(message: string): void {
    ...
    // Send as system message for voice output only
    const shoutMessage: GeminiTypes.Live.ClientContent = {
      client_content: {
        turns: [
          {
            role: 'user',
            parts: [{ text: 'Say this motivational shout in an energetic coach voice (just say it, nothing else): "' + message + '"' }]
          }
        ],
        turn_complete: true
      }
