...
Be encouraging but direct. Focus on actionable advice.`;
  ...
  onAwake(): void {
    ...
    if (this.listeningWaveAnimation) {
      ...
      if (image && image.mainPass && image.mainPass.baseTex) {
        this.waveAnimatedTexture = image.mainPass.baseTex as AnimatedTextureFileProvider;
        // Stop animation initially
        if (this.waveAnimatedTexture.stop) {
  ...
  private setListeningAnimation(active: boolean): void {
    ...
    if (this.waveAnimatedTexture) {
      if (active) {
        this.waveAnimatedTexture.play(-1, AnimatedTextureFileProvider.PlaybackMode.Loop);
      } else {
        this.waveAnimatedTexture.pause();
      }
    }
