    ...
    var ProgressBarController = _classThis = class extends _classSuper {
        ...
        onAwake() {
            ...
            if (this.fillBar) {
                ...
                this.fillScreenTransform = this.fillBar.getComponent('Component.ScreenTransform');
                if (this.fillScreenTransform) {
                    // Image mode - use ScreenTransform anchors
                    this.useScreenTransform = true;
                    this.fillOriginalAnchors = Rect.create(this.fillScreenTransform.anchors.left, this.fillScreenTransform.anchors.right, this.fillScreenTransform.anchors.top, this.fillScreenTransform.anchors.bottom);
        ...
        applyProgress(progress) {
            // 1. Fill bar update
            if (this.useScreenTransform && this.fillScreenTransform && this.fillOriginalAnchors) {
                // IMAGE mode: adjust right anchor (left stays fixed, right moves)
                // At 0%: right = left (bar invisible)
                // At 100%: right = original right (full width)
                const left = this.fillOriginalAnchors.left;
                const originalRight = this.fillOriginalAnchors.right;
                const width = originalRight - left;
                const newRight = left + (width * Math.max(this.minFillScale, progress));
                this.fillScreenTransform.anchors.setCenter(new vec2((left + newRight) / 2, (this.fillOriginalAnchors.top + this.fillOriginalAnchors.bottom) / 2));
                this.fillScreenTransform.anchors.setSize(new vec2(newRight - left, this.fillOriginalAnchors.top - this.fillOriginalAnchors.bottom));
                // Debug log
                if (Math.abs(progress - this._lastLoggedProgress) > 0.1) {
                    print('[ProgressBar] progress=' + (progress * 100).toFixed(0) + '% → right=' + newRight.toFixed(3));
