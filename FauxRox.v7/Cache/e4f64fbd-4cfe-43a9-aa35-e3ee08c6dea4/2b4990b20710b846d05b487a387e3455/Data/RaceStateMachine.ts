...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  private readonly BURPEE_JUMP_DISTANCE: number = 50;  // 50cm forward = valid jump
  ...
  private readonly FORM_REMINDER_COOLDOWN: number = 5.0;  // seconds between reminders

  // Thresholds (in cm)
  private readonly BURPEE_DROP_THRESHOLD: number = 40;   // Head must drop 40cm
  ...
  private checkBurpeeForm(cameraY: number): void {
    ...
    switch (this._burpeeState) {
      case 'waiting_drop':
        ...
        } else if (this._stationProgress > 2 && this._burpeeGoodReps === 0) {
          // User has moved 2m without a single good burpee
      ...
      case 'waiting_rise':
        ...
        if (cameraY - this._burpeeDropY > this.BURPEE_DROP_THRESHOLD * 0.7) {
          // Record position when rise completes - jump starts here
          if (this.camTransform) {
          ...
          print('[FormDetect] Burpee: RISE detected, waiting for jump');
        }
        break;

      case 'waiting_jump':
        // Waiting for actual forward displacement (jump)
        if (this._burpeeJumpStartPos !== null && this.camTransform) {
          ...
          var jumpDist = Math.sqrt(dx * dx + dz * dz);

          if (jumpDist >= this.BURPEE_JUMP_DISTANCE) {
            this._burpeeGoodReps++;
            print('[FormDetect] Burpee: GOOD REP #' + this._burpeeGoodReps + ' (jump: ' + jumpDist.toFixed(0) + 'cm)');
  ...
  private playFormReminder(): void {
    ...
    if (this.formReminderSound && !isNull(this.formReminderSound)) {
      ...
      print('[FormDetect] Playing form reminder');
    }
  }
