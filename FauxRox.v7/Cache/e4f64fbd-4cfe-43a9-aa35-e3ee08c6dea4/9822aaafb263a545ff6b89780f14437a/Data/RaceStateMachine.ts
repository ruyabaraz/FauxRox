...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  /** Screen flash overlay for GO! effect */
  @input @allowUndefined screenFlashImage: Image;
  ...
  private readonly COUNTDOWN_ZOOM_DURATION: number = 0.25;
  private readonly COUNTDOWN_ZOOM_SCALE: number = 1.8;

  // Screen flash for GO!
  private _screenFlashing: boolean = false;
  private _screenFlashTime: number = 0;
  private readonly SCREEN_FLASH_DURATION: number = 0.3;
  ...
  onAwake(): void {
    ...
    // Hide screen flash initially
    if (this.screenFlashImage) {
      this.screenFlashImage.enabled = false;
    }
  ...
  private triggerScreenFlash(): void {
    if (!this.screenFlashImage) return;

    this.screenFlashImage.enabled = true;
    this.screenFlashImage.mainPass.baseColor = new vec4(1, 1, 1, 0.8);
    this._screenFlashing = true;
    this._screenFlashTime = 0;
  }

  private updateScreenFlash(dt: number): void {
    if (!this._screenFlashing || !this.screenFlashImage) return;

    this._screenFlashTime += dt;
    var t = Math.min(1, this._screenFlashTime / this.SCREEN_FLASH_DURATION);

    // Fade out
    var alpha = 0.8 * (1 - t);
    this.screenFlashImage.mainPass.baseColor = new vec4(1, 1, 1, alpha);
