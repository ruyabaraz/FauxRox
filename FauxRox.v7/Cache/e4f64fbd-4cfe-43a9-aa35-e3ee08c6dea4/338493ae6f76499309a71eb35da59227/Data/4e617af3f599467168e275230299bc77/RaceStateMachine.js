...
let RaceStateMachine = (() => {
    ...
    var RaceStateMachine = _classThis = class extends _classSuper {
        ...
        spawnAndEnterStation() {
            ...
            var playerPos = this.getPlayerPosition();
            var playerForward = this.getPlayerForward();
            // Add forward offset to match START line behavior
            // START uses floorPos (2-3m ahead from calibration) + spawnDistance
            // So we add 200cm offset to playerPos before passing to CourseManager
            ...
            print('[RaceStateMachine] spawnAndEnterStation: spawnRef=(' + spawnReferencePos.x.toFixed(0) + ', ' + spawnReferencePos.z.toFixed(0) + ')');
            print('[RaceStateMachine] spawnAndEnterStation: playerForward=(' + playerForward.x.toFixed(2) + ', ' + playerForward.z.toFixed(2) + ')');
