...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  private initHeartRateMonitor(): void {
    if (isNull(this.heartRateTracker)) {
      print('[RaceStateMachine] No HeartRateTracker linked');
      this._hrStatusMessage = 'HR Monitor not configured';
      return;
    }

    // Always register BPM update callback
    this.heartRateTracker.onBPMUpdate((bpm: number, zone: HRZone) => {
      this.onHeartRateUpdate(bpm, zone);
    });

    // If BLEConnectionUI is linked, show the connection dialog
    if (!isNull(this.bleConnectionUI)) {
      print('[RaceStateMachine] Using BLEConnectionUI for HR connection');
      this.bleConnectionUI.show((connected: boolean) => {
        if (connected) {
          print('[RaceStateMachine] HR Monitor connected via dialog');
          this._hrStatusMessage = 'HR Connected';
          if (this.hrStatusText) {
            this.hrStatusText.text = '[HR] Connected';
          }
        } else {
          print('[RaceStateMachine] HR Monitor disabled by user');
          this._hrStatusMessage = 'HR Disabled';
          // heartRateHUD is already disabled by BLEConnectionUI
        }
      });
      return;
    }

    // Fallback: legacy auto-scan behavior (no dialog)
    print('[RaceStateMachine] Using legacy HR auto-scan');

    // Monitor connection state
    this.heartRateTracker.onConnectionStateChange((state: HRConnectionState, message: string) => {
      this._hrStatusMessage = this.formatHRStatusBanner(state, message);
      if (this.hrStatusText && this._state === RaceState.IDLE) {
        this.hrStatusText.text = this._hrStatusMessage;
      }
    });

    // Auto-start scanning (wrapped in try-catch to prevent crash if BLE unavailable)
    try {
      this.heartRateTracker.startScan();
      print('[RaceStateMachine] HR monitor initialized, scanning...');
    } catch (e) {
      print('[RaceStateMachine] ERROR: Failed to start HR scan: ' + e);
      this._hrStatusMessage = 'HR Monitor unavailable';
    }
  }

  private formatHRStatusBanner(state: HRConnectionState, message: string): string {
    switch (state) {
      ...
      case HRConnectionState.CONNECTING:
        return '[HR] Connecting...';
      case HRConnectionState.DISCONNECTED:
        return '[HR] Not connected\nPinch to scan';
  ...
  private onHeartRateUpdate(bpm: number, zone: HRZone): void {
