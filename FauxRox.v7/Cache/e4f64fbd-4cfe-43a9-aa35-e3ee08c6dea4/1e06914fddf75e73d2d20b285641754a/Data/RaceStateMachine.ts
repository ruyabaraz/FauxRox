...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  /**
   * Start the first station (START) - already spawned by CourseSetup
   */
  private startFirstStation(): void {
    ...
    this._currentConfig = course.getStationConfig(0);

    if (!this._currentConfig) {
      print('[RaceStateMachine] ERROR: No START config');
      return;
    }

    // START is already spawned by CourseSetup after calibration
    // Just enter station mode
    ...
    if (this._currentConfig.mode === StationMode.ZONE_HIT) {
      if (this.handZoneDetector && this._currentConfig.motionType) {
        this.handZoneDetector.startDetection(
          ...
          null,
          null  // No fixed station position - camera-relative
        );
  ...
  private trackStationDistance(): void {
    // GPS tracking updates _stationProgress via callback
    if (this.useGPSTracking && this.locationTracker && this.locationTracker.isLocationReady()) {
      return;  // Already tracking via callback
    }
  ...
  private trackRunDistance(): void {
    // GPS tracking updates _runDistance via callback
    if (this.useGPSTracking && this.locationTracker && this.locationTracker.isLocationReady()) {
      return;  // Already tracking via callback
    }
