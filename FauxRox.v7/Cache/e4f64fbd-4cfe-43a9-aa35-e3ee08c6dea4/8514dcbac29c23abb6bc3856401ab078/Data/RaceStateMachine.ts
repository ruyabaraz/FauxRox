...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  /** Finish splits container - holds split text entries */
  @input @allowUndefined finishSplitsContainer: SceneObject;

  /** Prefab for split entry (Text component) */
  @input @allowUndefined finishSplitPrefab: ObjectPrefab;
  ...
  // Finish panel split entries
  private _finishSplitEntries: SceneObject[] = [];
  ...
  private populateFinishSplits(incompleteStations: string[]): void {
    ...
    this.clearFinishSplits();

    if (!this.finishSplitsContainer || !this.finishSplitPrefab) {
      print('[RaceStateMachine] No split prefab - using fallback');
      return;
    }

    var yOffset = 0;
    var lineHeight = 1.5;  // Adjust based on your text size
    ...
    for (var j = 0; j < this._splitNames.length; j++) {
      ...
      var tag = j === fastIdx ? ' ★' : j === slowIdx ? ' ▼' : '';
      var splitText = this._splitNames[j] + ': ' + dur + 's' + avgHR + tag;

      this.addFinishSplitEntry(splitText, yOffset, false);
      yOffset -= lineHeight;
    }

    // Add incomplete stations if any
    if (incompleteStations && incompleteStations.length > 0) {
      yOffset -= lineHeight * 0.5;  // Extra spacing

      for (var k = 0; k < incompleteStations.length; k++) {
        var incompText = '○ ' + incompleteStations[k];
        this.addFinishSplitEntry(incompText, yOffset, true);
        yOffset -= lineHeight;
      }
    }
  }

  private addFinishSplitEntry(text: string, yPos: number, isIncomplete: boolean): void {
    if (!this.finishSplitPrefab || !this.finishSplitsContainer) return;

    var entry = this.finishSplitPrefab.instantiate(this.finishSplitsContainer);
    entry.enabled = true;

    // Position
    var transform = entry.getTransform();
    var pos = transform.getLocalPosition();
    transform.setLocalPosition(new vec3(pos.x, yPos, pos.z));

    // Set text
    var textComp = entry.getComponent('Component.Text') as Text;
    if (textComp) {
      textComp.text = text;
      // Dim incomplete stations
      if (isIncomplete) {
        var color = textComp.textFill.color;
        textComp.textFill.color = new vec4(color.r, color.g, color.b, 0.5);
      }
    }

    this._finishSplitEntries.push(entry);
  }

  private clearFinishSplits(): void {
    for (var i = 0; i < this._finishSplitEntries.length; i++) {
      var entry = this._finishSplitEntries[i];
      if (!isNull(entry)) {
        entry.destroy();
      }
    }
    this._finishSplitEntries = [];
  }
