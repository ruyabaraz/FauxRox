...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  /** Rep progress ring for ZONE_HIT stations */
  @input @allowUndefined repProgressRing: ScriptComponent;
  ...
  resetRace(): void {
    ...
    // Hide rep progress ring
    if (this.repProgressRing) {
      (this.repProgressRing as any).hide();
    }
  ...
  private enterStationMode(): void {
    ...
    if (this._currentConfig.mode === StationMode.ZONE_HIT) {
      // Show rep progress ring
      if (this.repProgressRing) {
        (this.repProgressRing as any).show(this._stationRequirement);
      }
      ...
      if (this.handZoneDetector && this._currentConfig.motionType) {
        ...
        this.handZoneDetector.startDetection(
          ...
          (repCount: number) => {
            ...
            // Update rep progress ring
            if (this.repProgressRing) {
              (this.repProgressRing as any).setReps(repCount);
            }
