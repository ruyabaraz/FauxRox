...
let RaceStateMachine = (() => {
    ...
    var RaceStateMachine = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.finishSplitsPanel = this.finishSplitsPanel;
            ...
            // Station gate crossing detection (for non-START stations)
            this._waitingForGateCross = false;
            this._gatePos = null;
        ...
        onUpdate() {
            ...
            if (this._state === RaceState.RUNNING) {
                ...
                // Check for station gate crossing before entering station
                if (this._waitingForGateCross) {
        ...
        checkGateCrossing() {
            ...
            var dot = toPlayer.x * this._gateForward.x + toPlayer.z * this._gateForward.z;
            // Player has crossed gate when they are past it (dot < 0)
            if (dot < 0) {
                this.onGateCrossed();
            }
        }
        onGateCrossed() {
            print('[RaceStateMachine] Gate crossed! Entering station mode: ' + this._currentConfig.name);
            this._waitingForGateCross = false;
            this._gatePos = null;
            this._gateForward = null;
            // Reset camera tracking position for station
            this._lastPlayerPos = null;
            // Fade out station gate
            var course = this.cm();
            if (course) {
                course.fadeOutAndDestroy(() => {
                    print('[RaceStateMachine] Station gate faded out');
                });
            }
        ...
        spawnAndEnterStation() {
            ...
            // Wait for gate crossing before entering station mode
            var activeStation = course.getActiveStation();
            if (activeStation) {
                this._gatePos = activeStation.getTransform().getWorldPosition();
                var gateForward = activeStation.getTransform().forward;
                this._gateForward = new vec3(gateForward.x, 0, gateForward.z).normalize();
                this._waitingForGateCross = true;
                print('[RaceStateMachine] Waiting for gate crossing at ' + this._currentConfig.name);
            }
            else {
                // Fallback: enter immediately if no station found
                this.enterStationMode();
            }
        }
        enterStationMode() {
