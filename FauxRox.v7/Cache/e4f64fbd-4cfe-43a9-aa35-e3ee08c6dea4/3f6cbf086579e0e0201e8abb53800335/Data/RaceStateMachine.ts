...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  togglePause(): void {
    if (this._state === RaceState.RUNNING || this._state === RaceState.STATION) {
      ...
      if (this._pausedFromState === RaceState.STATION && this.handZoneDetector && this._currentConfig) {
        var savedRepCount = this.handZoneDetector.getRepCount();
        this.handZoneDetector.startDetection(
          this._currentConfig.motionType,
          (repCount) => {
        ...
        // Note: HandZoneDetector will start fresh, but _stationProgress is preserved
      }
      ...
      if (this.statusText) {
        if (this._pausedFromState === RaceState.RUNNING) {
          ...
          // Station name is now in stationNameText, so clear statusText
  ...
  private startFirstStation(): void {
    ...
    course.fadeOutAndDestroy(() => {
      ...
      if (this._currentConfig.runDistanceBefore > 0) {
        ...
        // Clear station name when running
