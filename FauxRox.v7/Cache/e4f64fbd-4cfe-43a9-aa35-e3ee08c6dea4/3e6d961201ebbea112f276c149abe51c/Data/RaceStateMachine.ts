...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  private onHeartRateUpdate(bpm: number, zone: HRZone): void {
    // Update UI - only show BPM in heartRateText
    if (this.heartRateText) {
      this.heartRateText.text = 'HEART RATE\n' + bpm.toString();
    }

    // hrZoneText disabled - user requested single text field
    // if (this.hrZoneText && this.heartRateTracker) {
    //   this.hrZoneText.text = this.heartRateTracker.getZoneName(zone);
    // }
