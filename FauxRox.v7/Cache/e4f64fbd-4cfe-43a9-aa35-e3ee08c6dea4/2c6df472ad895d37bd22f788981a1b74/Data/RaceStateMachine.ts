...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  private initGpsStatusMonitor(): void {
    ...
    // Check if method exists (safety check)
    if (typeof this.locationTracker.onGpsStatusChange !== 'function') {
      ...
      // Fallback: poll status periodically
      this.createEvent('UpdateEvent').bind(() => {
        if (this._state !== RaceState.IDLE) return;
        var status = this.locationTracker.getGpsStatus ? this.locationTracker.getGpsStatus() : 'CHECKING';
        var message = this.locationTracker.getGpsStatusMessage ? this.locationTracker.getGpsStatusMessage() : '';
        var newText = this.formatGpsStatusBanner(status, message);
        if (newText !== this._gpsStatusText) {
          this._gpsStatusText = newText;
          this.setUIIdle();
        }
      });
      return;
  ...
  private onUpdate(): void {
    ...
    if (this._state === RaceState.RUNNING) {
      ...
      // Check if run distance is complete → auto-transition to station
  ...
  private prepareForStation(index: number): void {
    ...
    // Highlight station and setup detector
    ...
    if (this._currentConfig.runDistanceBefore > 0) {
      ...
      // No run before this station (e.g., START) → enter station directly
      print('[RaceStateMachine] No run required, entering station directly: ' + this._currentConfig.name);
      this._runTarget = 0;
      this._runDistance = 0;
  ...
  /**
   * Called when run distance target is reached (auto-transition to station)
   * This handles the case where player completes run distance without proximity trigger
   */
  private onRunDistanceComplete(): void {
    var course = this.cm();
    var index = this._currentStationIndex;
    ...
    print('[RaceStateMachine] Run distance complete! ' + this._runDistance.toFixed(1) + 'm / ' + this._runTarget + 'm');
    ...
    // Clear run target so we don't trigger again
    this._runTarget = 0;
    this._runDistance = 0;
  ...
  /**
   * Common station entry logic used by both proximity trigger and distance completion
   */
  private enterStationMode(): void {
    var course = this.cm();
    var index = this._currentStationIndex;

    // Progressive reveal: show the station when entering
    if (course && course.progressiveReveal) {
      course.revealStation(index);
    }
    ...
    print('[RaceStateMachine] Entered station ' + index + ': ' + this._currentConfig.name);
