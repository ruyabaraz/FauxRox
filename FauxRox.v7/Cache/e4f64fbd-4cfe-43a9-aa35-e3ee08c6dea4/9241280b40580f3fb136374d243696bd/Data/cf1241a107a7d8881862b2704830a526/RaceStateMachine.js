...
let RaceStateMachine = (() => {
    ...
    var RaceStateMachine = _classThis = class extends _classSuper {
        ...
        getPlayerForward() {
            ...
            // Get camera's look direction and NEGATE it
            // In Lens Studio, rot*back returns +Z when camera looks at -Z
            // So we negate to get the actual look direction
            var rot = this.camTransform.getWorldRotation();
            var lookDir = rot.multiplyVec3(vec3.back());
            // Negate to get correct forward direction
            var flatForward = new vec3(-lookDir.x, 0, -lookDir.z).normalize();
            print('[RaceStateMachine] getPlayerForward: rot*back=(' + lookDir.x.toFixed(2) + ', ' + lookDir.y.toFixed(2) + ', ' + lookDir.z.toFixed(2) + '), flat=(' + flatForward.x.toFixed(2) + ', ' + flatForward.z.toFixed(2) + ')');
            return flatForward;
        }
        // ── Helpers ────────────────────────────────────────────────────────────────
