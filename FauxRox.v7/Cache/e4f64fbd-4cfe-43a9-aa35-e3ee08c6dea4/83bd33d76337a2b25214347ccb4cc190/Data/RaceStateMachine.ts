...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  onAwake(): void {
    ...
    // Hide HR display immediately on start - only show after BLE device selected
  ...
  private onBLEFlowComplete(): void {
    ...
    // Show HR display ONLY if BLE device connected
    if (this.heartRateText && this.heartRateTracker && this.heartRateTracker.isConnected) {
  ...
  private onHeartRateUpdate(bpm: number, _zone: HRZone): void {
    // Track for current split (display is handled by updateHeartRateDisplay polling)
  ...
  /** Polls HR tracker and updates display - called every frame for reliability */
  private updateHeartRateDisplay(): void {
    if (!this.heartRateTracker || !this.heartRateText) return;

    var bpm = this.heartRateTracker.currentBPM;
    if (bpm > 0) {
  ...
  private showTitle(): void {
    ...
    // hrZoneText always disabled - user doesn't want zone labels (WARM UP etc.)
  ...
  startRace(): void {
    ...
    // Hide HR status text, show HR display (but NOT zone text - user doesn't want WARM UP etc.)
    if (this.hrStatusText) {
      this.hrStatusText.getSceneObject().enabled = false;
    }
    if (this.heartRateText) {
  ...
  resetRace(): void {
    ...
    // hrZoneText stays disabled - user doesn't want zone labels
  ...
  private onUpdate(): void {
    ...
    // Always update HR display from tracker (polling approach - more reliable than callback)
