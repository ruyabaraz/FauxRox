...
export class RaceStateMachine extends BaseScriptComponent {
  ...
  private updateCountdown(dt: number): void {
    ...
    if (this.timerText && !this.timerText.getSceneObject().enabled) {
      this.timerText.getSceneObject().enabled = true;
  ...
  private setUIIdle(): void {
    if (this.statusText) this.statusText.text = 'FauxRox';
    // Show GPS status
    if (this.stationInfoText) {
      this.stationInfoText.getSceneObject().enabled = true;
      this.stationInfoText.text = this._gpsStatusText || '';
    }
    // Hide timer until race starts
    if (this.timerText) {
      this.timerText.getSceneObject().enabled = false;
    }
  ...
  private updateTimerUI(): void {
    ...
    // Ensure timer is visible during race
    if (!this.timerText.getSceneObject().enabled) {
      this.timerText.getSceneObject().enabled = true;
    }
