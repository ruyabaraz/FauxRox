...
export class RepProgressRing extends BaseScriptComponent {
  ...
  show(totalReps: number): void {
    ...
    if (!this._material && !isNull(this.ringMesh)) {
      this._material = this.ringMesh.mainMaterial;
      print('[RepProgressRing] Material ref: ' + (!isNull(this._material) ? 'exists' : 'NULL'));

      // Debug: list all properties on mainPass
      if (this._material) {
        const pass = this._material.mainPass as any;
        print('[RepProgressRing] mainPass type: ' + typeof pass);
        print('[RepProgressRing] pass.progress before: ' + pass.progress);
        pass.progress = 0.75;
        print('[RepProgressRing] pass.progress after: ' + pass.progress);

        // Try to find the parameter
        for (const key in pass) {
          if (key.toLowerCase().includes('progress') || key.toLowerCase().includes('color')) {
            print('[RepProgressRing] Found key: ' + key + ' = ' + pass[key]);
          }
        }
      }
    }
    ...
    this.updateTexts();
    this.setRingProgress(0.5);  // TEST: start at 50% to see if ring is visible
  ...
  private setRingProgress(progress: number): void {
    if (isNull(this.ringMesh)) {
      print('[RepProgressRing] ERROR: ringMesh is null!');
      return;
    }

    // Try multiple methods to set shader parameter
    const mesh = this.ringMesh as any;
    const mat = this.ringMesh.mainMaterial as any;
    const pass = this.ringMesh.mainPass as any;

    // Method 1: via mainPass
    if (pass) {
      pass.progress = progress;
      pass.ringColor = this.ringColor;
    }

    // Method 2: via material mainPass
    if (mat && mat.mainPass) {
      mat.mainPass.progress = progress;
      mat.mainPass.ringColor = this.ringColor;
    }

    // Method 3: check if getMaterial exists
    if (mesh.getMaterial) {
      const m = mesh.getMaterial(0);
      if (m && m.mainPass) {
        m.mainPass.progress = progress;
        m.mainPass.ringColor = this.ringColor;
      }
    }
