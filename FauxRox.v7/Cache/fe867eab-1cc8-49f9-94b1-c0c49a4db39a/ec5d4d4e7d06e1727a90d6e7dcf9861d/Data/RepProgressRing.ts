...
export class RepProgressRing extends BaseScriptComponent {
  ...
  onAwake(): void {
    print('[RepProgressRing] onAwake - ringMesh: ' + (this.ringMesh ? 'exists' : 'NULL'));

    // Clone material for unique instance
    if (this.ringMesh) {
      print('[RepProgressRing] ringMesh.mainMaterial: ' + (this.ringMesh.mainMaterial ? 'exists' : 'NULL'));
      if (this.ringMesh.mainMaterial) {
