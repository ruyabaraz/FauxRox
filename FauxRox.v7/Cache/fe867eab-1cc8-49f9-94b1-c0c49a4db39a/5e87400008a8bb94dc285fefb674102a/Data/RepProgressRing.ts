...
export class RepProgressRing extends BaseScriptComponent {
  ...
  show(totalReps: number): void {
    ...
    // Get material reference (no clone - single instance)
    ...
    if (!isNull(this.ringMesh)) {
      print('[RepProgressRing] ringMesh.sceneObject.enabled: ' + this.ringMesh.getSceneObject().enabled);
      print('[RepProgressRing] ringMesh.enabled: ' + this.ringMesh.enabled);
    }
