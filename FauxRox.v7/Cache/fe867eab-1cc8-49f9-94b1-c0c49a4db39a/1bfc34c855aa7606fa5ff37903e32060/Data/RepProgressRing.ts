...
export class RepProgressRing extends BaseScriptComponent {
  ...
  onAwake(): void {
    ...
    if (this.ringMesh) {
      print('[RepProgressRing] ringMesh found: ' + this.ringMesh.getSceneObject().name);
      print('[RepProgressRing] mainMaterial: ' + (this.ringMesh.mainMaterial ? 'exists' : 'null'));
    }
  }
  ...
  private setRingProgress(progress: number): void {
    ...
    // Try mainMaterial.mainPass for Graph Materials
    const mat = this.ringMesh.mainMaterial;
    if (mat && mat.mainPass) {
      const pass = mat.mainPass as any;
      pass.progress = progress;
      pass.ringColor = this.ringColor;
      print('[RepProgressRing] Progress set via mainMaterial: ' + progress.toFixed(2));
    } else {
      // Fallback to direct mainPass
      const pass = this.ringMesh.mainPass as any;
      pass.progress = progress;
      pass.ringColor = this.ringColor;
      print('[RepProgressRing] Progress set via mainPass: ' + progress.toFixed(2));
    }
  }
