...
let CloudManager = (() => {
    ...
    var CloudManager = _classThis = class extends _classSuper {
        constructor() {
            ...
            this._userUUID = '';
            this._displayName = '';
            /** Set to true to disable Supabase in Preview mode */
            this.disableInPreview = this.disableInPreview;
        }
        __initialize() {
            super.__initialize();
            this.supabaseProject = this.supabaseProject;
        ...
        onAwake() {
            ...
            // Skip Supabase init in Preview (OIDC only works on device)
            if (this.disableInPreview && !global.deviceInfoSystem.isSpectacles()) {
                this.log('Supabase disabled in Preview mode (OIDC requires device)');
                return;
            }
            this.initClient();
        }
        ...
        // ── Save Race ───────────────────────────────────────────────────────────────
        async saveRace(record, isGuest = false) {
