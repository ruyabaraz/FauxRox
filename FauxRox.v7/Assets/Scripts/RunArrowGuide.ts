...
export class RunArrowGuide extends BaseScriptComponent {
  ...
  @input arrowHeightOffset: number = -80;  // 80cm below camera = waist level
   ...
   */
  startGuide(targetPos: vec3): void {
  ...
  private spawnArrowsAhead(): void {
    ...
    if (this._lastSpawnPos !== null) {
      ...
      // If player moved past last spawn point, calculate next spawn distance
      if (distFromLast > this.arrowSpacing * 0.5) {
        startDist = 100;  // Spawn new arrow 1m ahead
      } else {
        // Calculate where next arrow should be
        startDist = this.arrowSpacing - distFromLast;
        if (startDist < 100) startDist = 100;  // Minimum 1m ahead
      }
    }
  ...
  private spawnArrow(position: vec3, direction: vec3): void {
    ...
    // Rotate 180 on Y to flip arrow direction, then -90 on X to lay flat
    var flipRot = quat.fromEulerAngles(-90 * MathUtils.DegToRad, 180 * MathUtils.DegToRad, 0);
    var finalRot = forwardRot.multiply(flipRot);
