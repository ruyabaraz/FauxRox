...
export class CourseSetup extends BaseScriptComponent {
  ...
  private onCalibrated(pos: vec3, rot: quat): void {
    ...
    this.showStatus("");
    this.showInstruction("Pinch Start to begin. Wrist menu let's you pause/resume/stop.");
  ...
  /**
   * Put the START line back in front of the athlete without re-running floor
   * detection. Loading a new session destroys whatever was placed, so the
   * "race again with a different session" path needs a way to replace it.
   *
   * Uses where the athlete is standing and facing now rather than the original
   * calibration pose — on a second run they have usually moved.
   */
  respawnStartLine(camera?: SceneObject): boolean {
    var cm = this.getCourseManager();
    if (!cm || !this._isCalibrated) return false;

    var pos = this.getPlayerGroundPosition();
    if (!pos) return false;

    var forward = new vec3(0, 0, -1);
    if (camera) {
      var camForward = camera.getTransform().forward;
      forward = new vec3(camForward.x, 0, camForward.z).normalize();
    }

    cm.spawnStationInFrontOfPlayer(0, pos, forward);

    if (this.startButtonObject) {
      this.startButtonObject.enabled = true;
    }

    print('[CourseSetup] START line respawned');
    return true;
  }
