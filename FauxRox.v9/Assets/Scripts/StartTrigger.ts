...
export class StartTrigger extends BaseScriptComponent {
  ...
  /** Start button Interactable component (Image + Interactable) */
  @input @allowUndefined startButton: Interactable;

  /** Start button Image component for texture swap */
  @input @allowUndefined startButtonImage: Image;

  /** Normal state texture */
  @input @allowUndefined startButtonNormalTex: Texture;

  /** Hover state texture */
  @input @allowUndefined startButtonHoverTex: Texture;
  ...
  private bindButtonInteraction(): void {
    // Bind start button (Interactable)
    if (this.startButton) {
      try {
        this.startButton.onTriggerEnd.add(() => {
          this.onStartButtonPressed();
        });

        // Hover texture swap
        if (this.startButtonImage && this.startButtonHoverTex) {
          this.startButton.onHoverEnter.add(() => {
            this.startButtonImage.mainPass.baseTex = this.startButtonHoverTex;
          });
        }
        if (this.startButtonImage && this.startButtonNormalTex) {
          this.startButton.onHoverExit.add(() => {
            this.startButtonImage.mainPass.baseTex = this.startButtonNormalTex;
          });
        }

        print('[StartTrigger] Start button bound (Interactable)');
      } catch (e) {
  ...
  private updateHint(): void {
    ...
    if (this.pauseButtonObject) {
      var showPause = (state === 'RUNNING' || state === 'APPROACHING_STATION' || state === 'APPROACHING_FINISH' || state === 'STATION' || state === 'PAUSED');
