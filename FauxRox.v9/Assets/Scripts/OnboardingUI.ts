...
export class OnboardingUI extends BaseScriptComponent {
  ...
  /**
   * The typed birth year.
   *
   * A field rather than a stepper, because reaching 1978 from 1990 was twelve
   * pinches in mid-air. The stepper's inputs are still declared and still
   * bound if they are wired, so a scene that has not been updated yet keeps
   * working - but the field is what this is for.
   */
  @input @allowUndefined birthYearInputField: ScriptComponent;
  ...
  private updateUI(): void {
    ...
    if (this.birthYearInputField) {
      var yearField = this.birthYearInputField as any;
      if (yearField.text !== undefined && !this._birthYearTyped) {
        yearField.text = this._birthYear.toString();
      }
    }
  ...
  /**
   * Watch the field, but do not act on every keystroke.
   *
   * Somebody typing 1978 passes through "1", "19" and "197", none of which is
   * a birth year and all of which would fail a validator. Fighting them at
   * every character is how a field becomes unusable; the text is read once,
   * on confirmation.
   */
  private bindBirthYearInput(): void {
    if (!this.birthYearInputField) return;

    var field = this.birthYearInputField as any;

    // Same defensive pair as the name field: UIKit exposes one or the other
    // depending on the version installed.
    var onEdited = (text: string) => { this._birthYearTyped = text || ''; };

    if (field.onTextChanged && field.onTextChanged.add) {
      field.onTextChanged.add(onEdited);
      this.log('Birth year input bound via onTextChanged');
    } else if (field.onValueChanged && field.onValueChanged.add) {
      field.onValueChanged.add(onEdited);
      this.log('Birth year input bound via onValueChanged');
    }

    // Ask for a numeric keyboard where the field offers one. Where it does
    // not, the parser refuses anything that is not four digits anyway.
    if (field.keyboardType !== undefined) field.keyboardType = 'number';
    if (field.inputType !== undefined) field.inputType = 'number';
  }

  /** Whatever is in the field right now, committed only on confirmation */
  private _birthYearTyped: string = '';

  private onYearMinus(): void {
    this._birthYear = Math.max(EARLIEST_BIRTH_YEAR, this._birthYear - 1);
    this._birthYearTyped = String(this._birthYear);
    this.updateUI();
  }

  private onYearPlus(): void {
    this._birthYear = Math.min(LATEST_BIRTH_YEAR, this._birthYear + 1);
    this._birthYearTyped = String(this._birthYear);
    this.updateUI();
  }
  ...
  private onConfirm(): void {
    ...
    // Read once, here. An unreadable field leaves the year that was already
    // there - the one loaded from their profile, or the default for somebody
    // new - rather than writing a half-typed number over it.
    this._birthYear = commitBirthYear(this._birthYearTyped, this._birthYear);
